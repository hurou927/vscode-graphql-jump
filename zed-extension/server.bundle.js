...
var GRAPHQL_KEYWORDS = "query|mutation|subscription|fragment|type|input|interface|enum|scalar|union";
function findDefinition(base, files) {
...
var SUFFIXES = ["Query", "Mutation", "Fragment", "Subscription"];
...
// src/server.ts
var log = (msg) => connection.console.log(msg);
...
connection.onInitialize((params) => {
  if (params.workspaceFolders?.length) {
    workspaceRoot = (0, import_url.fileURLToPath)(params.workspaceFolders[0].uri);
  } else if (params.rootUri) {
    workspaceRoot = (0, import_url.fileURLToPath)(params.rootUri);
...
connection.onInitialized(() => {
  connection.client.register(import_node.DidChangeWatchedFilesNotification.type, {
    watchers: [{ globPattern: "**/*.{graphql,gql}", kind: import_node.WatchKind.Create | import_node.WatchKind.Delete }]
  });
});
...
connection.onDefinition((params) => {
  ...
  return import_node.Location.create(
    (0, import_url.pathToFileURL)(result.file).toString(),
    import_node.Range.create(
      import_node.Position.create(result.line, result.col),
      import_node.Position.create(result.line, result.col)
    )
  );
});
